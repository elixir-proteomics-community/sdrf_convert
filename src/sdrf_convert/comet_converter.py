...
# 3rd party imports
import pandas as pd
from pyteomics.mass.unimod import Unimod # type: ignore
...
class CometConverter(AbstractConverter):
    ...
    def __init__(
        self,
        comet_params: str,
        max_variable_modification: int = 5,
    ...
    def convert_sample(self, row_idx: int) -> Tuple[str, str, str]:
        ...
        # set fragment bin tolerance and unit
        ...
        if fragment_mass_tolerance < 0.1:
            # this looks like high res ms/ms
            sample_config = self.set_param(
                sample_config,
                "fragment_bin_tol",
    ...
    def convert_via_cli(cls, cli_args: argparse.Namespace):
        ...
        converter = CometConverter(
            ...
            max_variable_modification=cli_args.max_variable_modifications,
    ...
    def add_cli_args(cls, subparsers: argparse._SubParsersAction):
        ...
        tool_parser.add_argument("comet_params", help="Comet params file, needs to be generated by Comet with the '-p' flag")

        tool_parser.add_argument(
              "--max-variable-modifications", default="5", type=int, action="store", metavar="X", help="Max. number of variable modifications per sample (will be applied to all var. modification)"
        )
        tool_parser.add_argument(
              "-o", "--config-folder", default="./", type=str, action="store", metavar="/path/to/save/comet.params", help="Folder to store the generated config files"
        )
        tool_parser.add_argument(
